...
_You can use a confidence interval cos clt, but vastly inappropriate to use your model for prediction or prediction intervals.)_
...
"A taxpayers’ group felt that tiny towns were likely to spend large amounts per person because they have such small financial bases."
_should be negative_
...
```{r}
model1 <- lm(Expend ~Population,data=exp)
...
```{r}
ols_regress(Expend ~Population,data=exp)
```
...
ols_regress(Expend ~Population,data=exp2)
